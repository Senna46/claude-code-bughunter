...
- **Built-in rules**: Detects common issues (SQL injection, eval, hardcoded secrets, etc.) via regex and Claude prompt integration
...
BugHunter includes several built-in detection rules that are checked both via regex pattern matching and by inclusion in the Claude analysis prompt:
...
4. **Built-in Rules**: Regex-based detection for common issues (SQL injection, eval, hardcoded secrets, etc.)
